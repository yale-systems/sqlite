    ...
    if( pMWin->eFrmType==TK_RANGE ){
      ...
      if( op==WINDOW_AGGINVERSE ){
        if( pMWin->eStart==TK_FOLLOWING ){
          windowCodeRangeTest(
              p, OP_Le, p->current_csar.csr, regCountdown, p->start.csr, lblDone
          );
        }else{
          windowCodeRangeTest(
              p, OP_Ge, p->start.csr, regCountdown, p->current_csar.csr, lblDone
          );
        }
      }else{
        windowCodeRangeTest(
            p, OP_Gt, p->end.csr, regCountdown, p->current_csar.csr, lblDone
        );
      }
    }else{
  ...
  ** cursor does not go past it to EOF. */
  ...
  switch( op ){
    case WINDOW_RETURN_ROW:
      csr = p->current_csar.csr;
      reg = p->current_csar.reg;
...
void sqlite3WindowCodeStep(
  ...
  s.current_csar.csr = pMWin->iEphCsr;
  csrWrite = s.current_csar.csr+1;
  s.start.csr = s.current_csar.csr+2;
  s.end.csr = s.current_csar.csr+3;
  ...
  ** for the main loop, and for each cursor (start, current and end). */
  if( pMWin->eFrmType!=TK_ROWS ){
    ...
    s.current_csar.reg = pParse->nMem+1; pParse->nMem += nPeer;
  ...
  ** address of the OP_Gosub is stored in local variable addrGosubFlush. */
  ...
  if( pMWin->eFrmType!=TK_RANGE && pMWin->eStart==pMWin->eEnd && regStart ){
    ...
    windowAggFinal(&s, 0);
    sqlite3VdbeAddOp1(v, OP_Rewind, s.current_csar.csr);
    windowReturnOneRow(&s);
    sqlite3VdbeAddOp1(v, OP_ResetSorter, s.current_csar.csr);
  ...
  if( regPeer && pOrderBy ){
    ...
    sqlite3VdbeAddOp3(v, OP_Copy, regPeer, s.current_csar.reg, pOrderBy->nExpr-1);
  ...
  if( pMWin->eStart==TK_FOLLOWING ){
    ...
    if( pMWin->eEnd!=TK_UNBOUNDED ){
      if( pMWin->eFrmType==TK_RANGE ){
        ...
        windowCodeRangeTest(&s, OP_Ge, s.current_csar.csr, regEnd, s.end.csr, lbl);
